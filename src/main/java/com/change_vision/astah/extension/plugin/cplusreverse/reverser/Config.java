...
public class Config {
	...
	//from Eclipse
//	public static final String CONFIG_PROPERTIES = "api_sample\\sample_doxygen_cplus\\doxygen_cplus\\Config.properties";
...
//		inputFile = new FileInputStream(filePath);
	...
	public static List getClassNameAboutForbidCreateAssociation() throws IOException {
		if (LanguageManager.isCPlus()) {
			return Arrays.asList(new Config(CONFIG_PROPERTIES).getValue("cplus_types_for_attribute").split(","));
		} else if (LanguageManager.isCSHARP()) {
			return Arrays.asList(new Config(CONFIG_PROPERTIES).getValue("csharp_types_for_attribute").split(","));
		} else if (LanguageManager.isJAVA()) {
			return Arrays.asList(new Config(CONFIG_PROPERTIES).getValue("java_types_for_attribute").split(","));
		}
		return null;
	}
}